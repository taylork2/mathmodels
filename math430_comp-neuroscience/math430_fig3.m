...
I0 = [2.05]; %initial current[nA]
A = 0.17; %SS Amplitude of wave [nA]
A=0.03;
...
alpha = wgn(length(t), 1, tau);
spikeStart = false; 
spikeTime = zeros(N, length(t));
...
for k=1:N
    ...
    for i=1:length(I0)
        ...
        for j=1:length(t)-1
            eta = randn*D;
    %         alpha(j) = exp(1)*t(j)/tau*exp(-t(j)/tau);
            Iapp(j+1) = (I0(i)+A*sin(2*pi*f*t(j))+eta*alpha(j))* H(j); %amplitude of I0
...
% 
%             figure
%             plot(t, Iapp,t,1.924*ones(size(t)),'--', 'Linewidth', 2);
%             xlabel('t (msec)');
%             ylabel('stimulus (nA)');
%             axis([(ti+500) T 0 2.5]);     
%             legend(['Iapp=', num2str(I0(i))], 'Threshold=1.924');
...
for k=1:N
    for j=1:length(t)
        if spikeTime(k,j)==1
            line([t(j),t(j)],[k, k+1],'LineWidth', 3)
        end
    end
end
axis([(ti+500) T 0 N+1]);
xlabel('t (msec)');
ylabel('trial #');
title('Raster Plot: Fluctuation Driven');
